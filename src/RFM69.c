...
//0-Persistent CSMA code
void zero_persistent_send(const void* buffer, uint8_t bufferSize){
    //printf("\nzero persistent send called\n");

    if (receiveDone()){
        //printf("\nReceive done. Returning");
        return;
    }
...
/*
    ...
    while (!canSend()){
        ...
        while ((millis() - wait_time_start) < backoff){
            //wait
            printf("\nwaiting");
        }
        printf("\ndone waiting");
